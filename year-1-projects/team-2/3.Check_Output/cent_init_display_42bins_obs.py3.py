...
def cent_show(ax1,i,ctd):
    ...
    for j in range(7):
        for i in range(6):
            if abs(ctd[j,i])>5.0:
                ax1.annotate("{:4.1f}".format(ctd[j,i]),xy=(i,j),ha='center',va='center',stretch='semi-condensed',fontsize=10)
    return pic1
...
###-- Plotting basics
fig, axs = plt.subplots(4,2)  ## (ny,nx)
fig.set_size_inches(5.2,10.)    ## (lx,ly)
...
for ii,ax in enumerate(axs.flat):
    if ii<km:
        vv=np.copy(obsctd[ii,:].reshape([7,6]))*100.; vv=vv[::-1,:]
