...
Updates
#######
1. Fixed setProp(), now it works for VNIR
2. dx now is an input parameter of setProp()
3. Now readSHDOMRad() only output for one VAA
...
def readSHDOMRad(filename,hl=19,nx=4096):
    ...
    VAA = np.array([muphi1[1]])
    VZA = np.rad2deg(np.arccos([muphi1[0]]))
    Rad = np.array([rad1[:,2]])
    return Rad,VZA,VAA,SZA,SAA
def setProp(scat_file, wavelength,fc_obj,nx=4096,dx=0.02):
    '''
    ...
    for n in range(nphase):
        ...
        f.write("%d"% nlg)
        albedov[n]=dat1[2]
        dat0=np.genfromtxt(scat_file, skip_header=7,max_rows=1)
        dat0 = dat0[1:]
        tmp_nlg = len(dat0)
        ...
        idx_line = 8
        while (tmp_nlg < nlg):
            dat1=np.genfromtxt(scat_file, skip_header=idx_line,max_rows=1)
            dat1 = dat1.reshape((-1,))
            dat0  = np.concatenate([dat0,dat1])
            tmp_nlg = len(dat0)
            idx_line +=1
            len1 = len(dat1)
            for i in range(len1):
    ...
    for i in range(nx):
        for k in range(nz):
            ...
            #if(k>=nz-1):
            extinct=extv[i] #/nk, no need vertical is 1km
            albd=albedov[iphase-1]
            #else:
            #    extinct=0
            #    albd=1.0
            
            f.write("%d   1  %d  %5.0f  %5.6f %5.6f %d\n" % (i+1, k+1, temp, extinct, albd, iphase))
    f.close()    
