...
# -*- coding: utf-8 -*-
"""
Main Program: PLOT MONTHLY PRECIPITATION FROM VIC OUPUT RESULTS
...
# files store output
data = np.array(pd.read_csv('output_data_from_VIC/precip.csv',header=0,delimiter=','))
lat = data[:,0]
lon = data[:,1]
...
plt.savefig('Precipitation_color.png',dpi=600)
plt.show()
plt.close()
